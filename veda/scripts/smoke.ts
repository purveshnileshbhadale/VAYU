...
function connect(kind: 'device' | 'dashboard', id: string): Peer {
  const ws = new WebSocket(`${URL}?kind=${kind}`);
  const inbox: any[] = [];
  ws.on('message', (raw) => inbox.push(JSON.parse(String(raw))));
  const send = (type: string, to: string, body: any, from = id) =>
    ws.send(JSON.stringify({ v: 1, type, from, to, body }));
  return { id, ws, inbox, send } as unknown as Peer;
}
...
export async function main() {
  const laptop = connect('device', 'device-laptop');
  const phone = connect('device', 'device-phone');
  const dash = connect('dashboard', 'dashboard');
  await sleep(150);
  ...
  // 2) consent-gated control: dashboard locks phone (requires consent gate)
  const controlId = randomUUID();
  dash.send('control.request', 'device-phone', { controlId, action: 'lock', permission: 'system.lock', consent: 'requested', args: {} });
  await sleep(300);
  const consentReq = phone.inbox.find((m) => m.type === 'control.consent.request');
  if (!consentReq) throw new Error('consent gate did not fire for lock');
  // user approves on the phone
  phone.send('control.consent.result', '', { controlId, ok: true });
  await sleep(300);
  const ctl = phone.inbox.find((m) => m.type === 'control.request' && m.body?.action === 'lock');
  ...
  phone.send('control.result', 'dashboard', { controlId, ok: true, data: 'locked' });
  await sleep(300);
  ...
  // 4) non-critical action (notify) skips the gate
  dash.send('control.request', 'device-laptop', { controlId: randomUUID(), action: 'send_notify', permission: 'notify.send', consent: 'granted', args: { title: 'VEDA', body: 'ping laptop' } });
  await sleep(300);
  const nb = laptop.inbox.find((m) => m.type === 'control.request' && m.body?.action === 'send_notify');
  if (!nb) throw new Error('non-critical control did not dispatch without gate');
